...
# Each method will have its name, description, and parameters.
API_METHODS = {
    "getHandshake": {
...
class CometApp(App):
    ...
    def execute_command(self, command, args):
        ...
        if command == "exit":
            ...
            if args:
                cmd_name = args[0]
                if cmd_name in API_METHODS:
                    method_info = API_METHODS[cmd_name]
                    output_log.write(f"Command: {cmd_name}")
                    ...
                    output_log.write(f"Unknown command: {cmd_name}")
            else:
                output_log.write("Available commands:")
                for cmd, info in API_METHODS.items():
                    output_log.write(f"  {cmd}: {info['description']}")
                output_log.write("")
                output_log.write("Type 'help <command>' for more details.")
            ...
            if args:
                ...
                if found_commands:
                    ...
                    output_log.write(f"No commands found matching '{keyword}'.")
            else:
